...
public class EventLogViewModel : BaseViewModel
{
    ...
    private void ImportExcluded()
    {
        ...
        if (dialog.ShowDialog() == true)
        {
            ...
            {
                if (uint.TryParse(line.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out uint eventId) &&
                    !_excludedEventIds.Contains(eventId))
                    _excludedEventIds.Add(eventId);
            }
        }
    }
    ...
    private void ExportExcluded()
    {
        ...
        if (dialog.ShowDialog() == true)
        {
            File.WriteAllLines(dialog.FileName,
                _excludedEventIds.Select(id => id.ToString(CultureInfo.InvariantCulture)));
        }
    }
